...
fiftyoneDegreesCollection* fiftyoneDegreesCollectionCreateFromMemory(
	...
	fprintf(stderr, "=== fiftyoneDegreesCollectionCreateFromMemory reader->current= %d\n", reader->current);
	fprintf(stderr, "=== fiftyoneDegreesCollectionCreateFromMemory reader->startByte= %d\n", reader->startByte);
	fprintf(stderr, "=== fiftyoneDegreesCollectionCreateFromMemory header.startPosition= %d\n", header.startPosition);

	// Validate the header and the reader are in sync at the correct position.
	if ((FileOffsetUnsigned)(reader->current - reader->startByte) != header.startPosition) {
		return NULL;
	}
...
reader, \
dataSet->header.t); \
fprintf(stderr, "dataSet->t == NULL == %d\n", dataSet->t == NULL); \
if (dataSet->t == NULL) { \
	return INVALID_COLLECTION_CONFIG; \
}
...
static StatusCode initWithMemory(
	...
	fprintf(stderr, "=== initWithMemory 1 ===\n");
	...
	fprintf(stderr, "=== initWithMemory 2 %d ===\n", status);
	...
	fprintf(stderr, "=== initWithMemory 3 %d ===\n", status);
	...
	*(uint32_t*)(&dataSet->header.strings.count) = stringsCount;
		...
		fprintf(stderr, "=== initWithMemory 3.1 ===\n");
	...
	*(uint32_t*)(&dataSet->header.profiles.count) = profileCount;
	...
	dataSet->graphsArray = fiftyoneDegreesIpiGraphCreateFromMemory(
		...
		fprintf(stderr, "=== initWithMemory 4 ===\n");
	...
	if (reader->lastByte != reader->current) {
		...
		fprintf(stderr, "=== initWithMemory 5 ===\n");
...
static StatusCode initInMemory(
	...
	fprintf(stderr, "initInMemory DataSetInitInMemory %d\n", status);
	...
	fprintf(stderr, "initWithMemory initInMemory %d\n", status);
...
static StatusCode initDataSetFromFile(
	...
	fprintf(stderr, "=== initDataSetFromFile 1 ===\n");
...
fiftyoneDegreesStatusCode fiftyoneDegreesIpiInitManagerFromFile(
	...
	fprintf(stderr, "=== fiftyoneDegreesIpiInitManagerFromFile 1 ===\n");

	DataSetIpi* dataSet = (DataSetIpi*)Malloc(sizeof(DataSetIpi));

	fprintf(stderr, "dataSet == NULL %d\n", dataSet == NULL);

	if (dataSet == NULL) {
		return INSUFFICIENT_MEMORY;
	}

	fprintf(stderr, "=== fiftyoneDegreesIpiInitManagerFromFile 2 ===\n");
	...
	if (status != SUCCESS || EXCEPTION_FAILED) {
		...
		fprintf(stderr, "=== fiftyoneDegreesIpiInitManagerFromFile 3 ===\n");
